...
export default function FormattedMessage({ content, isUser = false }) {
  ...
  const processContent = (text) => {
    ...
    processed = processed.replace(
      /^## (.+)$/gm,
      '<h3 class="text-lg font-semibold text-gray-800 mt-4 mb-2 first:mt-0">$1</h3>'
    )

    // Procesar subtítulos (### Subtítulo)
    processed = processed.replace(
      /^### (.+)$/gm,
      '<h4 class="text-base font-medium text-gray-700 mt-3 mb-1">$1</h4>'
    )
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('•') || line.startsWith('-') || line.startsWith('*')) {
        ...
        processedLines.push(
          `<li class="flex items-start"><span class="text-blue-600 mr-2 mt-0.5">•</span><span>${listItem}</span></li>`
        )
      } else {
        if (inList) {
    ...
    processed = processed.replace(
      /^(🎯|⚡|💡|📈|🚀|✅|❌|⚠️)\s*(.+)$/gm,
      '<div class="flex items-start space-x-2 my-2 p-2 bg-blue-50 rounded-lg"><span class="text-lg">$1</span><span class="text-gray-800">$2</span></div>'
    )

    // Envolver en párrafo si no hay etiquetas de bloque
    if (
      !processed.includes('<h3>') &&
      !processed.includes('<ul>') &&
      !processed.includes('<div class="flex">') &&
